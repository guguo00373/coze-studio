...
import (
	...
	"github.com/coze-dev/coze-studio/backend/infra/orm/impl/postgres"
...
func Init(ctx context.Context) (*AppDependencies, error) {
	...
	// Support database type switching via DATABASE_TYPE environment variable.
	...
	case "postgres", "postgresql":
		deps.DB, err = postgres.New()
		if err != nil {
			return nil, fmt.Errorf("init postgres db failed, err=%w", err)
		}
	case "mysql":
		deps.DB, err = mysql.New()
		if err != nil {
			return nil, fmt.Errorf("init mysql db failed, err=%w", err)
		}
	default:
	...
	err = config.Init(ctx, deps.DB, deps.OSS) // Depends on DB, Idgen and OSS initialization
	if err != nil {
