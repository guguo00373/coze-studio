...
func (g *KVStore[T]) Save(ctx context.Context, namespace, k string, v *T) error {
	...
	// Use GORM's Save method which handles both MySQL and PostgreSQL
	// First try to update, if no rows affected then insert
	res := g.db(ctx).Exec(
		`UPDATE kv_entries SET value_data = ? WHERE namespace = ? AND key_data = ?`,
		data, namespace, k,
	)
	if res.Error != nil {
		return fmt.Errorf("failed to update key %s: %w", k, res.Error)
	}
	...
	if res.RowsAffected == 0 {
		...
		if res.Error != nil {
			return fmt.Errorf("failed to insert key %s: %w", k, res.Error)
		}
	}
