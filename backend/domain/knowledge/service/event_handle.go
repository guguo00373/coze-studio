...
func (k *knowledgeSVC) batchProcessSlice(ctx context.Context, doc *entity.Document,
	...
	var ids []int64
	var err error
	// Image documents have a single slice created together with the document.
	// Reuse that ID so search-store document IDs stay consistent with MySQL.
	if doc.Type == knowledge.DocumentTypeImage {
		...
		if err != nil {
			return errorx.New(errno.ErrKnowledgeDBCode, errorx.KV("msg", fmt.Sprintf("find image slice failed, err: %v", err)))
		}
	...
	if len(ids) == 0 && len(cacheRecord.ProcessingIDs) == 0 {
	...
	if len(ids) < length {
		newIDs, err := k.genMultiIDs(ctx, length-len(ids))
		if err != nil {
			return err
		}
	...
	if doc.Type == knowledge.DocumentTypeImage {
		if len(parseResult) != 0 {
			...
			if len(slices) > 0 {
				...
				if len(ids) == 0 {
					return errorx.New(errno.ErrKnowledgeIDGenCode, errorx.KV("msg", "image slice id is empty"))
				}
