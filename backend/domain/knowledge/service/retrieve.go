...
func (k *knowledgeSVC) packResults(ctx context.Context, retrieveResult []*schema.Document) (results []*knowledgeModel.RetrieveSlice, err error) {
	...
	foundSliceIDs := map[int64]struct{}{}
	for _, slice := range slices {
		if slice != nil {
			foundSliceIDs[slice.ID] = struct{}{}
		}
	}
	for _, doc := range retrieveResult {
		id, err := strconv.ParseInt(doc.ID, 10, 64)
		...
		if _, found := foundSliceIDs[id]; found {
			continue
		}
		...
		documentID := parseDocumentIDFromExternalStorage(ext[fieldNameDocumentID])
		if documentID == 0 {
			continue
		}
		fallbackSlices, _, err := k.sliceRepo.FindSliceByCondition(ctx, &entity.WhereSliceOpt{DocumentID: documentID})
		if err != nil {
			logs.CtxWarnf(ctx, "find slice by document id failed: %v", err)
			continue
		}
		if len(fallbackSlices) == 0 || fallbackSlices[0] == nil {
			continue
		}
		if _, found := foundSliceIDs[fallbackSlices[0].ID]; found {
			continue
		}
		slices = append(slices, fallbackSlices[0])
		foundSliceIDs[fallbackSlices[0].ID] = struct{}{}
		sliceIDs[fallbackSlices[0].ID] = struct{}{}
		sliceScoreMap[fallbackSlices[0].ID] = sliceScoreMap[id]
	}
	for _, slice := range slices {
...
func parseDocumentIDFromExternalStorage(v any) int64 {
	switch t := v.(type) {
	case int64:
		return t
	case int:
	...
	case string:
		...
		return id
	default:
		id, _ := strconv.ParseInt(fmt.Sprint(t), 10, 64)
		return id
	}
}
