# Coze Studio Helm Chart (Harbor + 内部 PostgreSQL 版本)
...
secret:
  ...
  stringData:
    ...
    MODEL_API_KEY_0: "sk-your_openai_api_key"
...
helm install opencoze ./opencoze-harbor-external-postgresql \
  --namespace coze \
  -f ./opencoze-harbor-external-postgresql/values.yaml

# 升级
helm upgrade opencoze ./opencoze-harbor-external-postgresql \
  --namespace coze \
  -f ./opencoze-harbor-external-postgresql/values.yaml
...
| PostgreSQL | harbor.ubidirector.cn/coze/postgres:15 |
...
```bash
kubectl logs statefulset/opencoze-opencoze-pg-postgresql -n coze
